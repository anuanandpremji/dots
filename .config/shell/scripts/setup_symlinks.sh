...
# ── Fonts (copy, not symlink — rarely changed) ───────────────────────────────
echo "── Fonts ──"
if [[ -d "$DOTFILES/.local/share/fonts" ]]; then
    if [[ "$DRY_RUN" == false ]]; then
        ...
        ' _ {} +
        echo -e "${GREEN}[OK]${NC}   Fonts synced to ~/.local/share/fonts/"
    else
        echo -e "${YELLOW}[DRY-RUN]${NC} Would copy fonts to ~/.local/share/fonts/"
    fi
fi
